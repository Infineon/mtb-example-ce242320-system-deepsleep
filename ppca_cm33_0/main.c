 ...
 */
#define LOOP_DELAY_MS      1000  // Base delay between operations
...
#define PPCA_M1_VAR_ADDRESS 0x20000400  // PPCA Core 0 shared variable (this core)
#define PPCA_M3_VAR_ADDRESS 0x20000800  // PPCA Core 1 shared variable
...
********************************************************************************
* Summary:
* This is the main function for PPCA Core 0 in the System Deep Sleep example.
* It drives LED1 via EPU software events, updates shared memory for the main
* core to monitor, and enters coordinated deep sleep when ready:
*    1. Generate EPU software events to toggle LED1 (500 ms on/off)
...
*******************************************************************************/
/**
 * @brief Main function for PPCA Core 0
 *
 * This function implements the main application flow for PPCA Core 0:
...
int main(void)
{
    ...
    uint32_t *shared_var = (uint32_t *)PPCA_M1_VAR_ADDRESS;

    // Initialize shared variable to indicate core is starting
    *shared_var = 500;  // Different initial value to distinguish from core 1
    ...
    while(1)
    {
        ...
        Cy_PPCA_EPU_PU_T2_Generate_SW_Event(PU_T2_3_GPIO2_HW, PU_T2_3_GPIO2_INDEX, 0);
        Cy_SysLib_Delay(DEEP_SLEEP_DELAY_MS);

        // Generate event 1 to toggle LED (on state)
        Cy_PPCA_EPU_PU_T2_Generate_SW_Event(PU_T2_3_GPIO2_HW, PU_T2_3_GPIO2_INDEX, 1);
        Cy_SysLib_Delay(DEEP_SLEEP_DELAY_MS);
        ...
        *shared_var += 2;
        ...
        // Main core will coordinate when both cores should enter deep sleep
        if(*shared_var >= 520)
        {
            // Signal readiness for deep sleep by setting flag value
            *shared_var = 200;  // Special value indicating ready for deep sleep
            ...
            *shared_var = 500;  // Reset to initial value
